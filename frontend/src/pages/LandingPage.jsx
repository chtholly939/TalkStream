...
const features = [
  {
    ...
    description: "Keep direct chats, shared spaces, and recent messages close without the usual tab-hopping.",
  },
  {
    icon: Video,
    title: "One-click video rooms",
    description: "Move from a thread into a call when tone, timing, or context needs a little more bandwidth.",
  },
  {
    icon: Users,
    title: "People-first presence",
    description: "See who is around, manage friends, and pick up conversations without losing the thread.",
  },
];
...
const Feature = ({ icon: Icon, title, description }) => (
  <article className="glass-hover rounded-2xl p-5">
    <div className="mb-4 flex h-11 w-11 items-center justify-center rounded-xl" style={{ background: "oklch(var(--p) / 0.12)", color: "oklch(var(--p))" }}>
    ...
    <main className="min-h-screen overflow-hidden mesh-bg" style={{ color: "var(--text-primary)" }}>
      ...
      <section className="mx-auto grid w-full max-w-7xl items-center gap-10 px-4 pb-14 pt-6 sm:px-6 sm:pb-16 lg:min-h-[calc(100vh-80px)] lg:grid-cols-[1.02fr_0.98fr] lg:gap-12 lg:px-8 lg:pb-24">
        <div className="max-w-3xl text-center lg:text-left">
          <div className="mb-6 inline-flex items-center gap-2 rounded-full px-3 py-1.5 text-xs font-semibold" style={{ background: "oklch(var(--b2) / 0.82)", border: "1px solid var(--border)", color: "var(--text-secondary)" }}>
            <Sparkles size={14} style={{ color: "oklch(var(--p))" }} />
            <span className="truncate">Built for chats that turn into collaboration</span>
          </div>

          <h1 className="font-display text-4xl leading-[1.06] sm:text-5xl lg:text-6xl xl:text-7xl" style={{ color: "var(--text-primary)" }}>
            Talk, call, and stay in sync without the clutter.
          </h1>

          <p className="mx-auto mt-5 max-w-2xl text-sm leading-7 sm:mt-6 sm:text-lg sm:leading-8 lg:mx-0" style={{ color: "var(--text-secondary)" }}>
            TalkStream brings messages, friends, and video calls into one calm workspace for teams, classmates, and communities that need to move quickly.
          </p>
          ...
          <div className="mx-auto mt-8 grid max-w-xl grid-cols-1 gap-3 min-[420px]:grid-cols-3 sm:mt-10 lg:mx-0">
            {highlights.map((item) => (
              <div key={item.label} className="rounded-2xl p-4 text-center min-[420px]:text-left" style={{ background: "oklch(var(--b2) / 0.62)", border: "1px solid var(--border)" }}>
        ...
        <div className="relative mx-auto w-full max-w-xl lg:mr-0">
          <div className="absolute -left-4 top-10 hidden h-24 w-24 rounded-3xl sm:block" style={{ background: "oklch(var(--s) / 0.16)", transform: "rotate(-8deg)" }} />
          <div className="glass relative rounded-3xl p-3 shadow-2xl sm:rounded-[2rem] sm:p-5" style={{ boxShadow: "0 28px 80px rgba(0,0,0,0.24)" }}>
            <div className="rounded-2xl p-3 sm:rounded-[1.5rem] sm:p-4" style={{ background: "oklch(var(--b1) / 0.74)", border: "1px solid var(--border)" }}>
              <div className="mb-4 flex flex-col gap-3 min-[430px]:flex-row min-[430px]:items-center min-[430px]:justify-between">
                ...
                <div className="flex w-fit items-center gap-2 rounded-full px-3 py-1.5 text-xs font-semibold" style={{ background: "oklch(var(--p) / 0.12)", color: "oklch(var(--p))" }}>
              ...
              <div className="grid gap-3 md:grid-cols-[1fr_0.78fr]">
                <div className="space-y-3">
                  {previewMessages.map((message) => (
                    <div key={message.name} className="rounded-2xl p-3" style={{ background: "oklch(var(--b2) / 0.95)", border: "1px solid var(--border)" }}>
                      <div className="flex items-center gap-3">
                        <div className="relative flex h-10 w-10 items-center justify-center rounded-full font-bold" style={{ background: "oklch(var(--p) / 0.14)", color: "oklch(var(--p))" }}>
                          {message.name[0]}
                ...
                <div className="flex min-h-44 flex-col justify-between rounded-2xl p-4 sm:min-h-56" style={{ background: "linear-gradient(160deg, oklch(var(--p) / 0.18), oklch(var(--s) / 0.1))", border: "1px solid var(--border)" }}>
                  <div>
                    <div className="mb-5 flex h-12 w-12 items-center justify-center rounded-2xl" style={{ background: "oklch(var(--b1) / 0.72)" }}>
                      <Phone size={22} style={{ color: "oklch(var(--p))" }} />
                    </div>
                    <h3 className="font-display text-lg">Live call ready</h3>
                    <p className="mt-2 text-sm leading-6" style={{ color: "var(--text-secondary)" }}>Start a room from the chat when the conversation needs faces and voices.</p>
                  </div>
      ...
      <section className="mx-auto w-full max-w-7xl px-4 pb-16 sm:px-6 sm:pb-20 lg:px-8">
        <div className="mb-8 flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
          ...
          <div className="flex items-center gap-2 text-sm" style={{ color: "var(--text-secondary)" }}>
            ...
            Messaging, friends, and calls in one place
          </div>
        </div>
