...
export default function ThemeSelector({ collapsed = false }) {
          ...
          <div className="max-h-72 overflow-y-auto px-2 pb-3">
            {dark.length > 0 && (
              <>
                ...
                {dark.map((t) => (
                  <ThemeOption key={t} name={t} meta={THEME_META[t]} active={theme === t}
...
function ThemeOption({ name, meta, active, onSelect }) {
  return (
    <button onClick={onSelect} data-theme={name}
      ...
      <div className="flex gap-0.5 rounded-lg overflow-hidden flex-shrink-0 border border-white/10" style={{ width: 36, height: 20 }}>
        <div className="flex-1" style={{ background: "oklch(var(--p))" }} />
        <div className="flex-1" style={{ background: "oklch(var(--s))" }} />
        <div className="flex-1" style={{ background: "oklch(var(--a))" }} />
        <div className="flex-1" style={{ background: "oklch(var(--n))" }} />
      </div>
      <span className="flex-1 text-xs font-medium" style={{ color: "var(--text-primary)" }}>{meta.emoji} {meta.label}</span>
